...
class ExtremaEnumerable(MoreEnumerable[TSource_co], Generic[TSource_co, TSupportsLessThan]):
    ...
    _selector: Callable[[TSource_co], TSupportsLessThan]
    _lt_op: Callable[[TSupportsLessThan, TSupportsLessThan], bool]
    ...
    def _get_iterable(self) -> Iterable[TSource_co]:
        ...
        return _extrema_by(it, getter, self._selector, self._lt_op)
...
def _extrema_by(
    ...
    selector: Callable[[TSource], TSupportsLessThan],
    lt_op: Callable[[TSupportsLessThan, TSupportsLessThan], bool],
    ...
    for elem in it:
        key = selector(elem)
        if lt_op(key, extremum_key):
            extrema.restart()
            extrema.add(elem)
            extremum_key = key
        # assumes strict total order
        elif not lt_op(extremum_key, key):
            extrema.add(elem)
